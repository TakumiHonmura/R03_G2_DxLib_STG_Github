...
/// <returns></returns>
BOOL ChaekCollToRect(RECT a, RECT b)
{
	if (a.left < b.right &&
			a.top < b.bottom &&
			a.right > b.left &&
...
//三角形を求めよう　
	float a = maru1.center.x - maru1.center.x;		//三角形の底辺の長さを求める
...
/// <param name="botton">下</param>
/// <returns></returns>
RECT GetRect(int left, int top, int right, int botton)
{
	//一時的にRECT型の変数を作って
	RECT rect = { left,top,right,botton };
...
VOID DrawRext(RECT r, unsigned int color, bool b)
{
	//引数を基に描画
