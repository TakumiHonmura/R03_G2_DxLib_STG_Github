...
struct MOUSE
{
	...
	int WheelValu = 0;					//マウスホイールの回転量(奥はプラス値 / 手前はマイナス値)
...
extern VOID MouseUpdete(VOID);									//マウスの入力情報を更新する
extern BOOL MouseDown(int MOUSE_INPUT);							//ボタンを押しているか、マウスコードで判断する
extern BOOL MouseUp(int MOUSE_INPUT);							//ボタンを押し上げたか、マウスコードで判断する
...
extern BOOL MouseRectClick(MARU rect, int MOUSE_INPUT_);		//マウスが矩形領域をクリックしたか
